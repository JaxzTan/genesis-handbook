import type { Metadata } from "next";
import { SiteNav } from "@/components/site-nav";
import { FeedbackForm } from "@/components/feedback/feedback-form";

export const metadata: Metadata = {
  title: "Feedback · Genesis",
  description: "Tell us what to improve in the Genesis hackathon handbook.",
};

export default function FeedbackPage() {
  return (
    <>
      <SiteNav variant="light" active="feedback" />
      <main className="min-h-dvh bg-g-off flex items-center justify-center px-6 pt-28 pb-16">
        <div className="w-full max-w-[640px]">
          <div className="mb-9">
            <div className="font-mono text-[12px] tracking-[0.2em] uppercase text-g-accent mb-4">
              We&apos;re listening
            </div>
            <h1
              className="font-serif font-bold text-g-ink leading-[1.0] tracking-[-0.03em]"
              style={{ fontSize: "clamp(40px, 6vw, 64px)" }}
              dangerouslySetInnerHTML={{ __html: "Tell us what to <em>fix.</em>" }}
            />
            <p className="text-[16px] text-g-mid leading-relaxed max-w-[52ch] mt-5">
              Genesis is written by the community. That includes telling us
              what&apos;s missing, confusing, or just wrong. Anonymous is fine.
            </p>
          </div>

          <FeedbackForm />
        </div>
      </main>
    </>
  );
}
